...
def get_or_create_token(user):
	...
	if token.count() >= 1:
		...
		if not token.is_expired():
			return token
		token.delete()
	phrase = user.email + ';' + timezone.now().strftime('%Y-%m-%d-%H-%S-%f')
	hashphrase = hashlib.sha256(bytes(phrase, 'ascii')).hexdigest()
	token = Token.objects.create(user=user, hash=hashphrase)
	return token
